...
export default function ValidateConfigComponent({ user }: ValidateConfigProps) {
        ...
        if (!existEquipment) {
            ...
            if (!existPackage) {
                ...
                if (!existData) {
                    ...
                    existPackage.data_with_selection_code.push(initialData);
    ...
    const handleCancel = useCallback((itemKey: string) => {
        ...
        // Step 1: Remove the added item from the equipmentList
        setEquipmentList((prevList) => {
            // Remove the specific equipment with the matching itemKey
            return prevList.map(equipment => {
                if (equipment._id === _id) {
                    // Filter out the config that matches the package8digit and package_selection_code
                    equipment.config = equipment.config.map(config => {
                        if (config.package8digit === package8digit) {
                            // Remove the data_with_selection_code that matches the package_selection_code
                            config.data_with_selection_code = config.data_with_selection_code.filter(data => data.package_selection_code !== package_selection_code);
                        }
                        return config;
                    }).filter(config => config.data_with_selection_code.length > 0); // Remove empty config
                }
                return equipment;
            }).filter(equipment => equipment.config.length > 0); // Remove empty equipment
        });

        // Step 2: Remove the itemKey from editKeys (the set of items in edit mode)
        setEditKeys((prevEditKeys) => {
            const updatedEditKeys = new Set(prevEditKeys);
            updatedEditKeys.delete(itemKey); // Remove the itemKey
            ...
            <Grid2 container spacing={2} sx={{ mb: 3 }}>
                ...
                <Grid2 size={2}>
                    <TextField
                        ...
                        disabled={autocompleteOptions.package8digit.some(c => c === selectionState.package8Digit)}
                    >
                                    ...
                                    <li key={config.package8digit}>
                                        ...
                                        <ul>
                                            {config.data_with_selection_code.map((data) => {
                                                ...
                                                return (
                                                    <Grid2 key={data.package_selection_code} container spacing={1}>
                                                        <Grid2 size={12} sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                                                            ...
                                                            <Button variant='outlined' size='small' color='inherit' onClick={() => handleCancel(itemKey)}>cancel</Button>
                                                        </Grid2>
                                                        <Grid2 size={12}>
                                                            <TextField fullWidth multiline value={JSON.stringify(data, null, 2)} />
                                                        </Grid2>
                                                    </Grid2>
                                                )
                                            })}
