...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const onConnect = () => {
            ...
            console.log(`Subscribing to status topics for equipment: ${selectedEquipment}`);
            subscribe(`equipments/status/communication_state/${selectedEquipment}`, (msg: string) => {
                console.log(`Received communication state: ${msg}`);
                setEquipmentStatus(prev => ({ ...prev, communicationState: msg }));
            });
            subscribe(`equipments/status/control_state/${selectedEquipment}`, (msg: string) => {
                console.log(`Received control state: ${msg}`);
                setEquipmentStatus(prev => ({ ...prev, controlState: msg }));
            });
            subscribe(`equipments/status/process_state/${selectedEquipment}`, (msg: string) => {
                console.log(`Received process state: ${msg}`);
                setEquipmentStatus(prev => ({ ...prev, processState: msg }));
            });
            subscribe(`equipments/status/process_program/${selectedEquipment}`, (msg: string) => {
        ...
        const onMessage = (message: string, receivedTopic: string) => {
            console.log(`Received message: ${message} on topic: ${receivedTopic}`);
            if (receivedTopic.includes(selectedEquipment)) {
                if (receivedTopic.includes('communication_state')) {
                    setEquipmentStatus(prev => ({ ...prev, communicationState: message }));
                } else if (receivedTopic.includes('control_state')) {
                    setEquipmentStatus(prev => ({ ...prev, controlState: message }));
                } else if (receivedTopic.includes('process_state')) {
                    setEquipmentStatus(prev => ({ ...prev, processState: message }));
                } else if (receivedTopic.includes('process_program')) {
                    setEquipmentStatus(prev => ({ ...prev, processProgram: message }));
                }
            }
        };

        // Connect to MQTT
        client = connectMqtt(onConnect, onError, onMessage);

        return () => {
            // Disconnect MQTT client when the component unmounts or equipment changes
            if (client && client.isConnected()) {
                client.disconnect();
            }
        };
    }, [selectedEquipment]); // Reconnect when equipment changes
